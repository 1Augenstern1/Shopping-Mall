...
export default {
  ...
  mounted() {
    this.$store.dispatch("getGoddsInfo", this.$route.params.skuId);
  },
  computed: {
  ...
  methods: {
    ...
    async addShopcar() {
      try {
        ...
        // 可以在query中传递所有信息但是搜索栏比较复杂，也可以通过本地存储的方式（h5新增（本地纯属（持久化有上限）和会话存储（非持久化，会话结束就消失）等））
        // 一些简单的数据可以通过query来传递，比较复杂的可以通过会话存储传递
        // 本地存储|会话存存储 （一般存储字符串）对象一般要把其转化为字符串 通过JSON方法
      ...
      // async函数一定返回的是盘promise{要么成功，要么失败}  await接收发送成功的数据（返回的数据有可能是成功的数据，也有可能是失败的数据）
      // 虽然派发了action也向发送数据到服务器,但是不知道结果
      // 通过接收返回的promise对象的值来判断成功与否
    },
  },
};
</script>
